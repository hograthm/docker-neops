...
def get_containers(client) -> List[Container]:
    ...
    try:
        ...
        for container in container_list:
            ...
            port_str = ', '.join([f"{k.split('/')[0]}->{v[0]['HostPort']}" if v else k.split('/')[0] for k, v in ports.items() if v])
            container_obj = Container(
                ID=container_info['Id'][:12],
                Image=container_info['Config']['Image'],
                Command=' '.join(container_info['Config']['Cmd']) if container_info['Config']['Cmd'] else 'N/A',
                CreatedAt=container_info['Created'],
                Status=container_info['State']['Status'],
                Ports=port_str,
                Names=container_info['Name'],
                Networks=', '.join(container_info['NetworkSettings']['Networks'].keys())
            )
...
def display_containers(containers: List[Container], compact: bool) -> None:
    ...
    for index, container in enumerate(containers):
        ...
        status = colored(container.Status, 'red' if 'exited' in container.Status.lower() else 'green')
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Docker Container Info Display")
    parser.add_argument('-c', '--compact', action='store_true', help="Enable super compact mode")
